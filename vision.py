...
def visFind(template, min_score, background):
    ...
    GUI.full_rotVal = int(GUI.full_rot.get())
...
def motion(event):
    ...
    if x <= 240 and y <= 320:
        update_entry_field(VisX1PixEntryField, x)
        update_entry_field(VisY1PixEntryField, y)
    elif x > 240:
        update_entry_field(VisX2PixEntryField, x)
    elif y > 320:
        update_entry_field(VisY2PixEntryField, y)
